# Hi, I'm Godwin Binoy

A self-taught developer who loves and supports open-source tools. I enjoy customizing and optimizing my own system, exploring command-line environments, and keeping up with the fast-paced evolution of technology.

---

### The Journey

* **Age 11 (The Start):** My path began with Google's Grasshopper (which shut down in 2023), where I picked up JavaScript and basic programming logic. From there, I moved into web development and then learned Python.
* **Age 15 (The Transition):** I permanently switched to Linux, which changed how I interact with computers. I began exploring different distros and command-line interfaces, and learning to tweak and optimize systems.
* **Present:** Currently, I am focused on learning C++ and Rust while staying updated with the latest technological shifts, including local AI and modern development workflows.

---

### What I Actually Know & Use

* **Core Languages:** Python, C++
* **Primary Skills:** Command-line (CLI) scripting, system customization, and system optimization.
* **Daily OS:** Fedora Workstation (GNOME).
* **Past Experience:** Web development.

---

### What I Have Tried & Explored

I enjoy experimenting with new tech trends and tools as they emerge.

* **Custom OS:** Made custom Fedora silverblue BlueBuild optimized for Intel's Meteor lake, Lunar lake and Arrow lake cpu architectures.
* **Android App Development:** Touched basic Java and Kotlin.
* **Lower-Level Programming:** Worked with a small part of C.
* **Operating Systems:**
  * Desktop Linux: Arch, Ubuntu, Lubuntu, Zorin.
  * Command-Line Linux: Alpine, Debian, Arch.
* **Local AI Models:** Running and optimizing open-weight AI models locally up to 14 Billion parameters.
* **Vibe Coding:** Gaining experience with modern, AI-assisted development (vibe coding) to prototype quickly and explore new ideas.
